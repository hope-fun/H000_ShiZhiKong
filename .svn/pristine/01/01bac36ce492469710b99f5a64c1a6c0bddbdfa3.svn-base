...
<p:fullurl value="${searchFilterAction}" var="cardViewSearchURL"/>	
<c:if test="${not empty requestType or requestType == 'ajax' }">
	<cards:listgroupcards searchData="${searchPlayerCardVO }" searchUrl="${cardViewSearchURL }" items="${playerCards}"></cards:listgroupcards>
</c:if>	
<c:if test="${empty requestType}">
	<div class="content cg030303">
		<div class=" menu-medium title-label-03">
					<span class="label-title-attack"><spring:message
							code="label.cg030303.title"></spring:message></span>
		</div>
								...
								<div class="div1">
									...
									<div class="card-major-image-equipment" >
										<div>
											...
											<c:if test="${ empty backPlayerCard.playerEquipmentId1 }">
										...
										<div>
											...
											<c:if test="${ empty backPlayerCard.playerEquipmentId2 }">
													<img src="<p:localurl value="/resources/images/icons/default.png"/>"/>
											</c:if>
										</div>
										<div>
											...
											<c:if test="${ empty backPlayerCard.playerEquipmentId3 }">
													<img src="<p:localurl value="/resources/images/icons/default.png"/>"/>
											</c:if>
										</div>
										<div></div>
		...
		<div id="cardSelectType">
			<span class="fc-title-01"><spring:message code="label.cg030303.surplusdemand"></spring:message></span> 
			<span><c:out value="${surplus}" /></span>
			<span class="fc-title-01">/</span>
			<span class="fc-title-01"> <c:out value="${playerVo.attackPowerMax}" /></span>
		</div>
		...
		<c:if test="${empty playerCards }">
			<div class="align-center">没有符合条件的卡牌</div>
		</c:if>
	...
	<c:if test="${not empty  backPlayerCard}">
		<input type="hidden" value ='/cg0303/cg030303/addCard/${playerCardGroupId}/${backPlayerCard.id }' id="urlPath"/>
	</c:if>
	<c:if test="${ empty  backPlayerCard}">
		<input type="hidden" value ='/cg0303/cg030303/addCard/${playerCardGroupId}' id="urlPath"/>
	</c:if>	
	</div>
</c:if>
