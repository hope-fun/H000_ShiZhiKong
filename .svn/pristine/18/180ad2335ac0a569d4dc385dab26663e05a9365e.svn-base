...
(function($) {
    ...
    var processbar = (function() {
		var processbar = function(elID, opts) {
            ...
            var widthAttr = that.attr("processbar");
            if(widthAttr == "") {
            	widthAttr = 0;
            }
            var fill = parseFloat(widthAttr) > 0;
            var full = parseFloat(widthAttr) == 100;
			var h = "<div class='processbar-child'>" + " <div "+ (that.attr("noNum")=="true"?"style=\"width:100%;\"":"style=\"width:70%;\"")
					...
					+ (full ? "class='full'" : "") + "></td>"
					+ "         </tr>" + "     </table>" + " </div>"
					+ (that.attr("noNum")=="true"?"":"<div style=\"width:30%; \">" + "    <span class='font'>" + widthAttr
					+ "%</span>" + " </div>") + "<div></div>" + "</div>";
            that.addClass("processing-bar").append(h);
			//动画效果
			setTimeout(function(){
				that.find(".target").css3Animate({width:widthAttr+"%",time:"150ms"});
			},100);
        };
        return processbar;
    })();
