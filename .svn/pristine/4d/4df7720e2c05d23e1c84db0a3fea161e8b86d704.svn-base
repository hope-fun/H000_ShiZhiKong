...
function strengthRequireOnClick() {
	kpyxTips("<spring:message code="label.cg1401.cardgrouparrange.content1"/>"
			+"<spring:message code="label.cg1401.cardgrouparrange.content2"/>"
			+"<spring:message code="label.cg1401.cardgrouparrange.content3"/>"
			+"<spring:message code="label.cg1401.cardgrouparrange.content4"/>"
			+"<spring:message code="label.cg1401.cardgrouparrange.content5"/>");
}
</script>
<div class="content cg030301">
	...
	<div class="tab-panel03">
		<div>
			<img alt=""
				src='<p:localurl value="/resources/images/buttons/zhandouzhenrong-yellow-b.png"/>'
				onclick='location.href="<p:fullurl value="/cg0303/cg030301/index"/>"'
				class='clickTone1' />
		</div>
		<div>
			<img alt=""
				src='<p:localurl value="/resources/images/buttons/hudunzhenrong-blue-b.png"/>'
	...
	<div class="container">
				...
				<c:forEach items="${playerCardGroups }" var="playerCardGroup"
					...
					<div>
											...
											<div class="cardgroup-info">
												<div>
													<div class="gitem icon icon-physicalstrength">
														<span><a class="fc-normal-01-l" href="#" onclick="strengthRequireOnClick()"><spring:message
																code="label.cg030201.strengthrequire"></spring:message></a></span>
														<span><c:out
														...
														<span class="fc-title-01">/</span> <span
															...
															<%-- <c:choose>
																<c:when test="${playerCardGroup.isRecommend == false }">
																	<c:out value="${playerVo.attackPowerMax}"></c:out>
																</c:when>
																<c:otherwise>
																	<c:out value="${playerVo.attackPower}"></c:out>
																</c:otherwise>
															</c:choose> --%>
						...
						<c:if
							...
							<div class="div4">
								<span class='fc-normal-06-s'><spring:message
										code='label.cg030301.zhandoukazu_msg'></spring:message></span><a
									onclick="location.href='<p:fullurl value="/cg0601/cg060101/index"></p:fullurl>';event.preventDefault();"><span
									class='fs-underline fc-normal-04-s'><spring:message
							...
							<div class="gbutton-01 ">
								...
								<div>
									<img
										...
										class='clickTone1'
										onclick='location.href="<p:fullurl value="/cg0303/cg030302/index/${playerCardGroup.id }"/>"' />
								</div>
								<div class="clearfix"></div>
							</div>
						</c:if>
						<c:if
							...
							<div class="gbutton-03">
								<img
									...
									onclick='location.href="<p:fullurl value="/cg0303/cg030301/raiseordernumber/${playerCardGroup.id }/${playerCardGroups.get(idx.index).orderNumber }/${playerCardGroups.get(0).orderNumber}"/>"'
									class='clickTone1' />
							</div>
						</c:if>
		...
		<c:if test="${fn:length(playerCardGroups) < 6}">
			<div class="editContainer">
				<div class="div5">
					<div class="panel-header02">
						<span><spring:message code="label.cg030301.editattackgroup"></spring:message>
							<c:out value="${fn:length(playerCardGroups)}"></c:out></span>
					</div>
				</div>
				<div class="borderPanel">
					<div class="panelA-header"></div>
					...
					<div class="border-content-noleft">
						<div class="edit-cardgroup">
							<img
								...
								class='clickTone1'
								onclick='location.href="<p:fullurl value="/cg0303/cg030302/index/${editPlayerCardGroupId}"/>"' />
						</div>
					</div>
				</div>
