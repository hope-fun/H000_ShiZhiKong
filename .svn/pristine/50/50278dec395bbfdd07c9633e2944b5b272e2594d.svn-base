				...
				<div class="npc-message">
					...
					<div class='npc-info fc-normal-05-m'>
						<div class="div1">
							...
							<c:out value="${beforePlayerCard.step}" />&rarr;
							<c:out value="${playerCard.step}" />
						</div>
						<div class="div2">
							<span class="fc-normal-02-l"> <spring:message
									code="label.cg030204.attack"></spring:message>：
							</span>
							<c:out value="${beforePlayerCard.attack}" />&rarr;
							<c:out value="${playerCard.attack}" />
						</div>
						<div class="div3">
							<span class="fc-link"> <spring:message
									code="label.cg030204.defence"></spring:message>：
							</span>
							<c:out value="${beforePlayerCard.defence}" />&rarr;
							<c:out value="${playerCard.defence}" />
						</div>
						<div class="div3">
							<span class="fc-normal-08-m">								
								<spring:message code="label.cg030210.cardevolution.cardrank"></spring:message>
							</span>
							<span class="fc-normal-03-m"><c:out value="${rank}" /></span>
							<c:if test="${rank <= beforeRank }">
								<img class="img-arrow" src='<p:localurl value="/resources/images/greenArrow.png"/>'/>
								<span class="fc-normal-08-m"><c:out value="${beforeRank - rank}" /></span>
							</c:if>
							<c:if test="${rank > beforeRank }">
								<img class="img-arrow" src='<p:localurl value="/resources/images/redArrow.png"/>'/>
								<span class="fc-normal-08-m"><c:out value="${rank - beforeRank}" /></span>
							</c:if>
						</div>
						<div class="div3">
							<span class="fc-normal-08-m">								
							...
							<span class="fc-normal-03-m"><c:out value="${totalRank}" /></span>
							<c:if test="${totalRank <= beforeTotalRank }">
								<img class="img-arrow" src='<p:localurl value="/resources/images/greenArrow.png"/>'/>
								<span><c:out value="${beforeTotalRank - totalRank}" /></span>
							</c:if>
							<c:if test="${totalRank > beforeTotalRank }">
								<img class="img-arrow" src='<p:localurl value="/resources/images/redArrow.png"/>'/>
								<span><c:out value="${totalRank - beforeTotalRank}" /></span>
							</c:if>
						</div>
					</div>
					<div></div>
