...
-(void)animationOfScrollOnlyOne
{
    CGPoint lb1Origin = self.label1.frame.origin;
    CGSize lb1Size = self.label1.frame.size;
//    NSLog(@"aaaaaaaaaaaaaaaaaa%f",self.label1.frame.origin.x);
//    NSLog(@"bbbbbbbbbbbbbbbbbb%f",self.label1.frame.size.width);
//    NSLog(@"cccccccccccccccccc%@",self.label1.text);
    if (self.animationOption == -1) {
        self.animationOption = UIViewAnimationOptionCurveLinear;
    }
    ...
    [UIView setAnimationDuration:self.timeInterval];
    if (self.label1.frame.origin.x == 0) {
        [self.label1 setFrame:CGRectMake(self.label1.frame.origin.x -  lb1Size.width, lb1Origin.y, lb1Size.width, lb1Size.height)];
    }else{
        [self.label1 setFrame:CGRectMake(self.label1.frame.origin.x -  lb1Size.width - RollFrame.size.width, lb1Origin.y, lb1Size.width, lb1Size.height)];
    }
    [UIView setAnimationDelegate:self];
    [UIView commitAnimations];
//    [UIView animateWithDuration:self.timeInterval delay:0 options:self.animationOption animations:^{
//        NSLog(@"aaaaaaaaaaaaaaaaaa%f",self.label1.frame.origin.x);
//        if (lb1Origin.x == 0) {
