...
<script>
	function changeCardGroup(form, raidId) {
		...
		form.action = '<p:fullurl value="/cg1803/cg180303/updateCardGroup"/>'
				+ '/' + raidId + '/' + selectId;
		form.submit();
	}
																		...
																		if ((idx + 1) >= len) {
																			if (viewdata.result
																				...
																				if (viewdata.data.fightResult == "FAILURE") {
																					window.location.href = '<p:fullurl value="/cg1803/cg180305/index/"/>'
																							+ viewdata.data.jumpUrlParameter;
																				} else if (viewdata.data.fightResult == "WIN") {
																					window.location.href = '<p:fullurl value="/cg1803/cg180304/index/"/>'
																							+ viewdata.data.jumpUrlParameter;
																				}																			
																				//talking data	
																				var bossId="${raid.description}"+"${raid.name}";																					
...
<div class="content">
	...
	<form id="attackForm"
		action="<p:fullurl value="/cg1803/cg180303/fight/" />">
