			...
			function worldChatJsExchange() {
				...
				window
						.eval("if(typeof timeInterval != \"undefined\")"
								...
								+ "});"
								+ "timeInterval = setInterval(\"messagePublisher.DoRefresh('<p:fullurl value='/cg0101/cg010102/getMessages'/>')\", 5000);");
			}
	...
	<script type="text/javascript">
		...
		function getReward(obj) {
			...
			$.get(url,function(json) {
				...
				if(json.isDiamond){
					var data = "{\"num\":"+json.diamondCount+",\"reason\":新手福利任务赠送钻石}";
				...
				if("${playerRegularTaskVO.getReward}" != null){
					if(json.isDiamond){
						var data = "{\"num\":"+ json.diamondCount +",\"reason\":日常任务赠送钻石}";
						JSInterface.getDiamondFromSystem(data);
					}
				}
			...
			var options = {
					...
					success : function(json) {
						...
						if (json.succeed) {
							...
							if(json.isDiamond){					
								var data = "{\"num\":" + json.diamondCount+ ",\"reason\":系统赠送钻石" + "}";
								JSInterface.getDiamondFromSystem(data);
							}
					...
					success : function(resp) {
						if (resp.succeed) {
						    var srin = 	'<div class="messageOfUseAction"><span><spring:message code="label.cg01010301.cost"></spring:message>'+resp.costActionPower+'<spring:message code="label.cg01010301.costattackpower"></spring:message></span><br></br><span><spring:message code="label.cg01010301.attackpoweradd"></spring:message>'
						               +resp.silverCoin+'<spring:message code="label.cg01010301.costcystal"></spring:message></span>'+resp.actionToExp+'<span><spring:message code="label.cg01010301.exp"></spring:message></span><br></br><span style="color:gray;">Tips:<spring:message code="label.cg01010301.addedinfo"></spring:message></span> </div>';
						   kpyxTips(srin);
...
<input type="hidden" id="speaktoofast"
	value="<spring:message code='label.cg0101.speaktoofast' />" />
<input type="hidden" id="cg0101inputnull"
	value="<spring:message code='label.cg0101.inputnull' />" />
<input type="hidden" id="illegalcharacter"
...
<input type="hidden" id="useConfirm"
...
<input type="hidden" id="fenpeicishu"
	value="${playerVO.actionPowerUseOutTimes}" />
<!-- 当请求类型为ajax且来自playerList时，更新此段 -->
<c:if
	test="${empty header['requestType'] or (header['requestType'] == 'ajax' and header['requestFrom'] == 'playerList') }">


	<div class="list-title">
		<img class="arrow" alt=""
			src="<p:localurl value="/resources/images/little-arrow-left.png"/>"
			onclick="clickLeftArrow()" />
		<c:if test="${empty listType or listType eq 'BEST_PLAYER'}">
			<span class="fc-normal-06-m"><spring:message
					code='label.cg01010301.rank.player' /></span>
		</c:if>
		<c:if test="${listType eq 'BEST_CARD'}">
			<span class="fc-normal-06-m"><spring:message
					code='label.cg01010301.rank.card' /></span>
		</c:if>
		<c:if test="${listType eq 'BEST_FIGHTER'}">
			<span class="fc-normal-06-m"><spring:message
					code='label.cg01010301.rank.fight' /></span>
		</c:if>
		<c:if test="${listType eq 'BEST_PHILANTHROPIST'}">
			<span class="fc-normal-06-m"><spring:message
					code='label.cg01010301.rank.charitarian' /></span>
		</c:if>
		<img class="arrow" alt=""
			src="<p:localurl value="/resources/images/little-arrow-right.png"/>"
			onclick="clickRightArrow()" />
	</div>
	<c:if test="${ not empty playerListInfoVOs }">
		<c:forEach items="${ playerListInfoVOs}" var="item" varStatus="status">
			<c:if
				test="${fn:length(fn:replace(item.id, item.listType, '')) eq 1}">
				<div class="list-item">
					<img 
			...
			<c:if
				test="${fn:replace(item.id, item.listType, '') eq playerVO.id }">
				<div class="my-list-rank">
					<%-- 	<span>我的排名：</span>
	...
	<c:if test="${empty playerListInfoVOs }">
		<div style="text-align: center;">
			<span><spring:message code='label.cg01010301.rank.more' /></span>
		</div>
	</c:if>
	<c:if test="${empty playerListInfoVOs }">
		<img class="btn-more2" alt=""
			src="<p:localurl value="/resources/images/border/more.png"/>"
			onclick='location.href="<p:fullurl value="/cg2301/cg230101/index"/>"' />
	</c:if>
	<c:if test="${ not empty playerListInfoVOs }">
		<img class="btn-more" alt=""
			src="<p:localurl value="/resources/images/border/more.png"/>"
			onclick='location.href="<p:fullurl value="/cg2301/cg230101/index"/>"' />
	</c:if>
	</div>
				...
				<c:forEach items="${msgList }" var="item" varStatus="itemStatus">
					<div class="msg-text">
						...
						<c:if test="${item.msgType eq 'FRIENDAPPLY' }">
							<a href="<p:fullurl value="/cg1101/cg110101/index"/>"><span>${item.name}</span></a>
							<span><spring:message code='label.cg01010301.friend.apply' /></span>
						</c:if>
						<c:if test="${item.msgType eq '0' }">
							<span><spring:message code='label.cg01010301.you' /></span>
							<a
								href="<p:fullurl value="/cg0501/cg050107/index/${item.eId }/homePage"/>"><span>${item.name}</span></a>
							<span><spring:message code='label.cg01010301.attacked' /></span>
						</c:if>
						<c:if test="${item.msgType eq '1' }">
							<span><spring:message code='label.cg01010301.youattack' /></span>
							<a
								href="<p:fullurl value="/cg0501/cg050107/index/${item.eId }/homePage"/>"><span>${item.name}</span></a>
						</c:if>
					</div>
	...
	<div class="panel1-right hiddenForCG010103" id="0">
		<div class="card-label">
			<span class="label-text"><spring:message
					code='label.cg01010301.my.cardrank' /></span> <img
				class="clickTone1 img-change"
				src='<p:localurl value="/resources/images/qiehuan.png"/>' />
		</div>
		<div class="card-img">
			<c:forEach items="${playerCards }" var="item" varStatus="itemStatus">
				<c:if test="${empty item.id }">
					<c:set var="src">
						<p:localurl value="/resources/images/scard-smallcard.png" />
					</c:set>
				</c:if>
				<c:if test="${not empty item.id }">
					<c:set var="src">
						<p:cardImg type="small" id="${item.cardId }"></p:cardImg>
					</c:set>
				</c:if>
				...
				<c:if test="${itemStatus.first }">
					<c:if test="${not empty item.id }">
						<c:set var="src">
							<p:cardImg type="ls" id="${item.cardId }"></p:cardImg>
						</c:set>
					</c:if>
					<c:if test="${empty item.id }">
						<c:set var="src">
							<p:localurl value="/resources/images/specialcard.png" />
						</c:set>
					</c:if>
...
<!-- 当请求类型为ajax且来自cardGroup时，更新此段 -->
<c:if
	test="${header['requestType'] == 'ajax' and header['requestFrom'] == 'cardGroup'}">

	<c:forEach items="${playerCardGroupList }" var="pList"
		varStatus="pListStatus">
		<div class="panel1-right hiddenForCG010103"
			id="${pListStatus.index + 1 }">
			<div class="card-label">
				<c:if test="${fn:length(pList.name) > 5 }">
					<span class="label-text">${fn:substring(pList.name,0,5) }...</span>
				</c:if>
				<c:if test="${fn:length(pList.name) <= 5 }">
					<span class="label-text">${pList.name }</span>
				</c:if>
				<img class="clickTone1 img-change"
					src='<p:localurl value="/resources/images/qiehuan.png"/>' />
			</div>
			<div class="card-img">
				<c:forEach items="${pList.playerCardGroupCards }" var="item"
					...
					<c:if test="${not empty item.id }">
						<c:set var="src">
							<p:cardImg type="small" id="${item.playerCard.cardId }"></p:cardImg>
						</c:set>
					</c:if>
					<c:set var="classValue">clickTone1 img img-small</c:set>
					<c:if test="${itemStatus.first }">
						...
						<c:if test="${not empty item.id }">
							<c:set var="src">
								<p:cardImg type="ls" id="${item.playerCard.cardId }"></p:cardImg>
							</c:set>
						</c:if>
						<c:set var="classValue">clickTone1 img</c:set>
					</c:if>
					...
					<c:if test="${not empty item.id }">
						<img src='${src }' class="${classValue }"
							onclick="location.href='<p:fullurl value="/cg0301/cg030102/index/${item.playerCardId }"/>'" />
					</c:if>
					<c:if test="${empty item.id }">
						<img src='${src }' class="${classValue }" />
					</c:if>
			...
			<!-- 将总攻击及防御力隐藏，点击转换卡组时加载至对应位置 -->
			<input type="hidden" id="attack${pListStatus.index + 1 }"
				value="${pList.totalAttackAdded }" /> <input type="hidden"
				id="defence${pListStatus.index + 1 }"
				value="${pList.totalDefenceAdded }" />
		</div>
	</c:forEach>
</c:if>
...
<c:if test="${empty header['requestType']}">
	...
	<div class="panel-attackanddefence">
		<div class="totalOfAttack">
			<img
				src="<p:localurl value="/resources/images/totalAttack/zongzhandouli.png"/>">
			<div class="totalAttackNum" id="totalOfAttack"></div>
		</div>


		<div class="totalOfDefence">
			<img
				src="<p:localurl value="/resources/images/totalDefence/zonghudunli.png"/>">
			<div id="totalOfDefence" class="totalDefenceNum"></div>
		</div>
		<div class="Mydetail">
			<span class="fc-normal-06-m fs-underline"
				onclick="location.href='<p:fullurl value="/cg0101/cg010106/index?totalAttackAdded=${totalAttackAdded }&totalDefenceAdded=${totalDefenceAdded }"/>'"><spring:message
					code='label.cg01010301.my.detail' /></span>
		</div>
	</div>
	<div class="clearfix"></div>
	<!-- 当请还有未分配的属性点时，更新此段 -->
...
<c:if
	...
	<div class="tab-task hiddenForCG010103">
		<div class="tabpanel">
			<div class="tab">
				<img class='clickTone1 img-task'
					src='<p:localurl value="/resources/images/buttons/meirirenwu-yellow-b.png"/>' />
			</div>
			<div class="tab">
				<img class='clickTone1 img-activity'
					src='<p:localurl value="/resources/images/buttons/huodong-blue-b.png"/>' />
			</div>
			<div class="number_pop tab">
				<c:if test="${giftMsg>0 }">
					<div class="giftMsgPop">
						<span>N</span>
					</div>
				</c:if>
				<img class='clickTone1 img-reward'
					src='<p:localurl value="/resources/images/buttons/jiangli-blue-b.png"/>' />
			</div>
			<div class="tab">
				<img class='clickTone1 img-chat' id="123456"
					src='<p:localurl value="/resources/images/buttons/liaotian-blue-b.png"/>' />
			</div>
			<div class="clearfix"></div>
			<div class="split">
						...
						<div class="task-footer">
							...
							<c:if test="${playerFreshmanTaskVO.day ne 7 }">
								...
								<c:if test="${nextFreshmanTaskVO.rewardType eq 'CARD' }">
									<img
										src='<p:localurl value="/resources/images/cards/tinysmall/TD2.png"/>'
										class="img-reward-item" />
									<span>${nextFreshmanTaskVO.rewardCount }</span>
								</c:if>
								<c:if
								...
								<c:if
									...
									<img
										src='<p:localurl value="/resources/images/consumable/small/YSB-S.png"/>'
										class="img-reward-item" />
									<span>${nextFreshmanTaskVO.rewardCount }</span>
								</c:if>
						...
						<div class="task-list regular-task">
							...
							<div class="task-list-3">
								<c:if
									...
									<span class="fc-normal-06-m fs-underline"
										onclick="location.href='<p:fullurl value="/cg0801/cg080102/index"/>'"><spring:message
											code='label.cg01010301.task.goto' /></span>
								</c:if>
								<c:if
...
<c:if test="${empty header['requestType']}">
	<div class="tab-activity hiddenForCG010103">
		<div class="tabpanel">
			<div class="tab">
				<img class='clickTone1 img-task'
					src='<p:localurl value="/resources/images/buttons/meirirenwu-blue-b.png"/>' />
			</div>
			<div class="tab">
				<img class='clickTone1 img-activity'
					src='<p:localurl value="/resources/images/buttons/huodong-yellow-b.png"/>' />
			</div>
			<div class="number_pop tab">
				<c:if test="${giftMsg>0 }">
					<div class="giftMsgPop">
		...
		<div class="activity-body">
			...
			<div style="margin-top: 15px" id="commonActivities">
			
				<div class="activity-img-left">
					<c:choose>
						<c:when test="${playerVO.level < 8}">
							<img
								src='<p:localurl value="/resources/images/activity/shouyehuodong01weikaifang.png"/>'>
						</c:when>
						<c:otherwise>
							<img class="clickTone1"
				...
				<div class="activity-img-right">
					<c:choose>
						<c:when test="${playerVO.level < 8}">
							<img
								src='<p:localurl value="/resources/images/activity/shouyehuodong02weikaifang.png"/>'>
						</c:when>
						<c:otherwise>
							<img class="clickTone1"
				...
				<div class="activity-img-left activity-img02">
					<c:choose>
						<c:when test="${playerVO.level < 20}">
							<img
								src='<p:localurl value="/resources/images/activity/shouyehuodong03weikaifang.png"/>'>
						</c:when>
						<c:otherwise>
							<img class="clickTone1"
								src='<p:localurl value="/resources/images/activity/shouyehuodong03.PNG"/>'
								onclick="location.href='<p:fullurl value="/cg1802/cg180201/index"/>'">
						</c:otherwise>
					</c:choose>
				</div>
				<div>
...
<c:if
	...
	<div class="tab-reward hiddenForCG010103">
		<div class="tabpanel">
			...
			<div class="number_pop tab">
				<c:if
					test="${not empty milestoneList||not empty giftList||giftMsg>0}">
					<div class="giftMsgPop">
						<span>N</span>
					</div>
				</c:if>
				<img class='clickTone1 img-reward'
					src='<p:localurl value="/resources/images/buttons/jiangli-yellow-b.png"/>' />
			</div>
			<div class="tab">
				<img class='clickTone1 img-chat'
					src='<p:localurl value="/resources/images/buttons/liaotian-blue-b.png"/>' />
														...
														<c:choose>
															...
															<c:otherwise>
																...
																<c:if
																	...
																	<div>
																		<span class="fc-title-02"><spring:message
																				code="label.cg1101.cg110104.gongneng"></spring:message>:</span><span
																			class="fc-content-03">${item.description.length()
																			> 15 ?
							...
							<div class="panelB-body panelB01-height">
								<div class="fc-container-02 fc-container-cg01010301">
									<c:forEach items="${dailyGiftList}" var="item" varStatus="k">
										...
										<c:if test="${item.type.name() eq 'RAID_KEY' }">
											<img src='<p:localurl value="/resources/images/consumable/middle/D9.png" />' />
										</c:if>	 --%>
...
<c:if test="${empty header['requestType']}">
	...
	<div class="tab-chat hiddenForCG010103">
		<div class="tabpanel">
			...
			<div class="tab">
				<img class='clickTone1 img-chat'
					src='<p:localurl value="/resources/images/buttons/liaotian-yellow-b.png"/>' />
			</div>
			<div class="clearfix"></div>
		</div>
		<div>
			...
			<div class="send-message-button"><img onclick="messagePublisher.DoPublish('<p:fullurl value='/cg0101/cg010102/publishMessage'/>')" src="<p:localurl value='/resources/images/buttons/fasong-green.png'/>" /> </div>
		</div>
	</div>
