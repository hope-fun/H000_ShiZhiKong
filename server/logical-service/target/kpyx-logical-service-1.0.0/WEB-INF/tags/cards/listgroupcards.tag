	...
	<tags:pagination items="${items}" id="searchCardList" method="GET" renderAjax="false" searchData="${searchData }" paginationUrl="${searchUrl }">
		<div>
			<c:forEach items="${items}" var="cardItem">
				<div class="panel-03">
					<div class='<cssutils:cardCamp campType="${cardItem.cardCamp}"></cssutils:cardCamp>'>
						<div class="panel-header">
							...
							<div class="item-stars-style">
								<div class="card-star">
									<c:forEach begin="1" step="1" end="${cardItem.star}" var="idx"><img alt="" src="<p:localurl value="/resources/images/star-d.png"/>"/></c:forEach>
								</div>
							</div>
							...
							<div class="card-assistant">
								<div class="small-card-width">
									<div class="card-assistant-image">
										<a href='<p:fullurl value="/cg0301/cg030102/cardDetail/${cardItem.id} "/>'>
											<img class="img-action" 
												src="<p:cardImg type="small" id="${cardItem.cardId }"></p:cardImg>" />
										</a>
									</div>
								</div>
									...
									<div class="card-assistant-desc">
										...
										<div class="bottom-right-panel">
											<div class="choose-button-width">
													<input id="add_card_${cardItem.id }" class="add_card" type="image" value="<c:out value='${cardItem.id }' />" src="<p:localurl value='/resources/images/buttons/jiaru-green.png'/>" />
												</div>
												<div class="clearfix"></div>
											</div>
											<div></div>
