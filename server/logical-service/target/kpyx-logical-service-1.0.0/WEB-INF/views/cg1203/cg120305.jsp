...
<div  class="content">
	<div class=" menu-medium title-label-03">
		<span><c:if test="${not empty targetUnionVO.name }">${targetUnionVO.name }</c:if></span>
			<input id="targetMirror" type="hidden" value="${vipSystemWarTargetMirrorViewDataList[0].id}" />
	...
	<union:targetMirror targetMirror="${vipSystemWarTargetMirrorViewDataList}"></union:targetMirror>
	<div class="conference-buttons clickTone1" >
		<img id="kaishigongji"   src='<p:localurl value="/resources/images/buttons/kaishigongji-green-z.png"/>' style="margin: 0 auto;display:<c:if test='${vipSystemWarTargetMirrorViewDataList[0].defence>0}'>block</c:if><c:if test='${vipSystemWarTargetMirrorViewDataList[0].defence<=0}'>none</c:if>"/>
	</div>
	
	<div class="conference-buttons clickTone1" >
		<img id="kaishigongjigray" src='<p:localurl value="/resources/images/buttons/kaishigongji-green-z.png"/>'  class="button_gray" style="margin: 0 auto;display: <c:if test='${vipSystemWarTargetMirrorViewDataList[0].defence<=0}'>block</c:if><c:if test='${vipSystemWarTargetMirrorViewDataList[0].defence>0}'>none</c:if>" >
	</div>

	<!--<div class="center-site">
		<span class="fc-normal-06-m system-content-other panleText" id="qingshiyong">*您当前战斗能量不足，请使用道具</span>
	</div>-->
	...
	<div class="check-fightresource">
		<div class="sellout-fr">
		<input name ="checkzhanhunengyuan" type="checkbox" id="checkzhanhunengyuan" class="sell_out" checked="false"/>
			</div>
			
			<div class="fightresource-label">
			<span class="fc-normal-06-m system-content-other panleText"><label id ="checkzhanhunengyuanlabel" for="checkzhanhunengyuan" class="checkbox-pic-style checkbox-checked"></label>*选中复选框，自动使用战斗药剂</span>
			</div>
	...
	<div class="center-site commonPading01 menu-medium" >
		<span><spring:message code="label.cg120305.vipwar.warfare" />: </span><span id="concumableAttackNumber">${concumableAttackNumber }</span>
	</div>
		...
		<c:if test="${not empty header['requestType'] or header['requestType'] == 'ajax' }">
			<div class="center-site commonPading01 menu-medium">
				<span><spring:message code="label.cg120305.vipwar.current.warfare" />: </span><span id="playerRealAttackPower">${cardGroupDivTagView.playerRealAttackPower}</span><span>/</span><span id="playerRealAttackPowerMax">${cardGroupDivTagView.playerRealAttackPowerMax}</span>
			</div>
...
<script type="text/javascript" charset="utf-8" >
			...
			if(${vipSystemWarTargetMirrorViewDataList[0].defence>0})
				{
				   img1Object=$("#img_${vipSystemWarTargetMirrorViewDataList[0].id}")
				}else{
					findCanClickStar();
				}
		$(function() {
...
function initInterval(warleftTime,isVip){
	...
	//if(isVip == "TRUE")
	//	{
	//		intervarlTimes = warStartTime + 3600*1000*24 -new Date().getTime();
	//	}else
	//	{
	//		intervarlTimes = warStartTime + 3600*1000*1 - new Date().getTime();
	//	}
...
function setRemainTime() { 
	...
	//		  day + "天" + hour + "小时" + minite + "分钟" + second + "秒");
		...
		window.clearInterval(interValObj);
	  	//跳转control
		window.location.href="<p:fullurl value='/cg1203/cg120305/encapAttackResult/'/>"+$("#warRecordsId").val()+"/FALSE";
	}
}
//函数确认是否已经获得胜利
function checkDestroy(){
	...
	for(var i=0;i<grays.length;i++)
		...
		if(number>=5)
			{
			//跳转control
			window.location.href="<p:fullurl value='/cg1203/cg120305/encapAttackResult/'/>"+$("#warRecordsId").val()+"/TRUE";
			}
}
...
function beginAttack() {
	...
	var cardGroupTotalAttack = $("#cardGroupTotalAttack").html();
		 if($("#defence").html()<=0)
			 {
			 kpyxTips("<spring:message code='label.cg120305.vipwar.statille.fail' />");
			 findCanClickStar();
				//如果所有的星球都被击败了跳转胜利页面
				   checkDestroy();
			 	return
		   ...
		   if( $("#checkzhanhunengyuan").attr('checked') == 'true')
			   {
				   if(eval(concumableAttackNumber) <=0 && eval(playerRealAttackPower) < eval(cardGroupAttackRequire))
			   		{
			   			kpyxTips("<spring:message code='label.cg120305.vipwar.use.warfare' />");
			   		 return 
			   		}else
			   		{
		...
		if(IsUseConsumable =="FALSE" && eval(playerRealAttackPower) < eval(cardGroupAttackRequire))
			{
			 	...
			 	kpyxTips("<spring:message code='label.cg120305.vipwar.replenish.warfare' />");	
		...
		if(cardGroupTotalAttack<=0)
			{
			    kpyxTips("<spring:message code='label.cg120305.vipwar.card.attack' />");	
				return
			}
			
			var url = "<p:fullurl value='/cg1203/cg120305/attackTargetMirror/'/>"+cardGroupTotalAttack+"/"+$("#targetMirror").val()+"/"+cardGroupAttackRequire+"/"+IsUseConsumable;
			//去除点击事件
			...
			$.post(url,randomParms,function (message){
				...
				if(null != message && true == message.result)
					...
					//VIP如果已经被占领
					if(message.data.isCaptrue == true)
						{
						...
						if(message.data.defence <=0)
							{
								...
								var imagegary="#imgdivgary_"+$("#targetMirror").val();
							 	...
							 	var image="#img1div_"+$("#targetMirror").val();
				  			    ...
				  			    img1Object = null;
				  			  kpyxTips("<spring:message code='label.cg120305.vipwar.statille.destroy' />");
				  			 //设置攻击按钮为不可攻击状态
...
function getTargetMirror(targetMirror){
	var id = targetMirror.id;
	if(null != id)
		{
	var imageType = targetMirror.id.split("_")[0];
	var mirrorId = targetMirror.id.split("_")[1];
	
	
	if(imageType=="img"){
		var url = "<p:fullurl value='/cg1203/cg120305/getMirror/'/>"+mirrorId;
		var randomParms = {anticache : Math.floor(Math.random()*1000)};
		$.post(url,randomParms,function (message){
